...
% Central place for defaults (update here only).
    ...
    % ===================== New parameters =====================
    % Thresholding mode toggle (kept as a single boolean in params)
    p.useGOtsu            = false;     % false = Adaptive (local), true = Global Otsu

    % Overlap splitting
    p.splitOverlapsEnable = false;     % checkbox
    p.splitMinCoreDistPx  = 3;         % px; minimum core-to-core distance for splitting

    % Line-bridging strengths
    p.strengthBridge      = 0.6;       % 0–1; higher = more aggressive bridging
    p.strengthShrink      = 0.2;       % 0–1; post-bridge shrink to trim overgrowth
end
