function visualize_cilia_masks(stack, uniqueDetections, params)
    ...
    if nargin < 3
        error('Requires inputs: stack, uniqueDetections, params.');
    end
    ...
    for i = 1:numDetections
        ...
        % Dilate for spread, then erode to remove padding around mask
        ...
        for c = 1:3
            ch = backgroundOverlay(:,:,c);
            ch(backgroundMask) = color(c);
            backgroundOverlay(:,:,c) = ch;
        end
    end
...
% %     end
% %     if ~isfield(params, 'backgroundPadding')
% %         params.backgroundPadding = 2; % Default spread in pixels
% %     end
% % 
